...
export default function Audits() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        {isManager && (
          <button
            ...
            className="px-4 py-2 rounded-sm bg-accent hover:bg-accent-hover text-white text-xs font-semibold flex items-center gap-2 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-accent/15"
          >
      ...
      {error && (
        <div className="p-4 rounded border border-danger/20 bg-danger-subtle text-danger text-xs font-medium flex items-center gap-2">
              ...
              <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                {cycles.map((c) => {
                  ...
                  return (
                    <div 
                      key={c.id} 
                      className={`p-12 rounded border transition-all cursor-pointer flex flex-col gap-1.5 ${
                        isSelected 
                    ...
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <span className={`px-8 py-0.5 rounded-full border text-[9px] font-bold ${
                          c.status === 'Open' 
                      ...
                      {isManager && (
                        <div className="flex gap-2 pt-2 border-t border-border/60 mt-1 justify-end">
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="px-2 py-1 rounded-sm border border-border text-[9px] font-semibold text-text-primary bg-surface hover:bg-neutral-subtle"
                          >
                            Discrepancies
                          </button>
                          {c.status === 'Open' && (
                            <button
                              ...
                              className="px-2 py-1 rounded-sm bg-danger text-white text-[9px] font-semibold hover:bg-danger/90"
                            >
                              Close Cycle
                            </button>
                          )}
        ...
        {selectedCycleId && (
          <div className="lg:col-span-8 space-y-4">
            <div className="p-6 bg-surface border border-border rounded-md space-y-4">
              ...
              {isDetailsLoading ? (
                <div className="p-12 text-center text-xs text-text-secondary flex justify-center items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin text-accent" />
                  <span>Loading scoped assets...</span>
                </div>
              ) : scopedAssets.length === 0 ? (
                <div className="p-8 text-center text-xs text-text-muted italic bg-[#F7F8FA] rounded">
                  No assets match this cycle's scope department or location criteria.
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Progress Indicator */}
                  <div className="p-3 rounded border border-border bg-[#F7F8FA] space-y-2">
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-[#F7F8FA] border-b border-border text-text-secondary font-bold select-none">
                          <th className="py-10 px-3">Asset</th>
                          <th className="py-10 px-3">Location</th>
                          <th className="py-10 px-3">Current Holder</th>
                          <th className="py-10 px-3">Audited State</th>
                          <th className="py-10 px-3 text-right">Actions</th>
                        </tr>
                      </thead>
                        ...
                        {scopedAssets.map((asset) => {
                          ...
                          return (
                            <tr key={asset.id} className="hover:bg-neutral-subtle/40 transition-colors">
                              ...
                              <td className="py-10 px-3 text-text-secondary font-medium">{asset.location}</td>
                              <td className="py-10 px-3">
                                {asset.currentHolder ? (
                              ...
                              <td className="py-10 px-3 text-right">
                                {isClosed ? (
                                  ...
                                  <button
                                    ...
                                    className="px-10 py-1.5 rounded-sm bg-accent hover:bg-accent-hover text-white text-[10px] font-semibold"
                                  >
                                    {log ? 'Re-audit' : 'Verify'}
                                  </button>
                                )}
      ...
      {isNewCycleOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-surface border border-border rounded shadow-xl overflow-hidden animate-fade-in">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-[#F7F8FA]">
            ...
            <form onSubmit={handleCreateCycle} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-surface border border-border rounded-md px-3 py-2 text-xs text-text-primary font-sans focus:outline-none focus:border-accent"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="border border-border rounded p-3 space-y-1.5 max-h-[150px] overflow-y-auto bg-surface-sunken">
                  {employees.map((emp) => (
                    <label key={emp.id} className="flex items-center gap-2 text-xs font-medium text-text-primary cursor-pointer hover:bg-neutral-subtle/50 p-1 rounded transition-colors">
                      <input
                        type="checkbox"
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setIsNewCycleOpen(false)}
                  className="px-4 py-2 border border-border hover:bg-neutral-subtle rounded-sm text-text-primary text-xs font-semibold transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-accent hover:bg-accent-hover text-white rounded-sm text-xs font-semibold flex items-center gap-1.5 shadow-sm transition-colors disabled:opacity-50"
                >
                  {isFormSubmitLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
      ...
      {loggingAsset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-surface border border-border rounded shadow-xl overflow-hidden animate-fade-in">
            ...
            <form onSubmit={handleLogItem} className="p-6 space-y-4">
              <div className="p-2 rounded bg-surface-sunken border border-border text-[11px] text-text-secondary space-y-0.5">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-surface border border-border rounded-md px-3 py-2 text-xs text-text-primary font-sans focus:outline-none focus:border-accent resize-none"
                ></textarea>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-accent hover:bg-accent-hover text-white rounded-sm text-xs font-semibold shadow-sm transition-colors"
                >
                  {isLogSubmitLoading && <Loader2 className="w-3.5 h-3.5 animate-spin inline mr-1.5" />}
      ...
      {reportCycleId && reportData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-3xl bg-surface border border-border rounded shadow-xl overflow-hidden animate-fade-in flex flex-col max-h-[85vh]">
            <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-[#F7F8FA] shrink-0">
              <h3 className="text-sm font-bold text-text-primary flex items-center gap-2">
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3 border border-border rounded bg-surface-sunken">
                  <div className="text-[10px] font-bold text-text-secondary uppercase">Scoped Assets</div>
                  <div className="text-lg font-bold text-text-primary mt-1">{reportData.summary.totalScoped}</div>
                </div>
                <div className="p-3 border border-border rounded bg-surface-sunken">
                  <div className="text-[10px] font-bold text-text-secondary uppercase">Verified Count</div>
                  <div className="text-lg font-bold text-success mt-1">{reportData.summary.verifiedCount}</div>
                </div>
                <div className="p-3 border border-border rounded bg-surface-sunken">
                  <div className="text-[10px] font-bold text-text-secondary uppercase">Missing Count</div>
                  <div className="text-lg font-bold text-danger mt-1">{reportData.summary.missingCount}</div>
                </div>
                <div className="p-3 border border-border rounded bg-surface-sunken">
                  <div className="text-[10px] font-bold text-text-secondary uppercase">Pending Count</div>
                  <div className="text-lg font-bold text-warning mt-1">{reportData.summary.pendingCount}</div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-danger flex items-center gap-1.5 border-b border-border pb-1">
                    <XCircle className="w-3.5 h-3.5" />
                    <span>Missing Assets ({reportData.discrepancies.missing.length})</span>
                  </h4>
                  {reportData.discrepancies.missing.length === 0 ? (
                    <div className="text-xs text-text-muted italic p-2">No assets marked missing.</div>
                  ) : (
                    <div className="border border-border rounded overflow-hidden">
                      <table className="w-full text-left text-xs border-collapse">
                        <tr className="bg-[#F7F8FA] border-b border-border font-bold">
                          <th className="py-1.5 px-3">Asset Tag</th>
                          <th className="py-1.5 px-3">Name</th>
                          <th className="py-1.5 px-3">Auditor Notes</th>
                        </tr>
                        {reportData.discrepancies.missing.map((item: any) => (
                          <tr key={item.id} className="border-b border-border">
                            <td className="py-1.5 px-3 font-mono font-semibold">{item.asset.assetTag}</td>
                            <td className="py-1.5 px-3 font-semibold text-text-primary">{item.asset.name}</td>
                            <td className="py-1.5 px-3 text-text-secondary italic">"{item.notes || 'No description provided'}"</td>
                          </tr>
                        ))}
                      </table>
                    </div>
                ...
                <div className="space-y-2 mt-4">
                  <h4 className="text-xs font-bold text-alert flex items-center gap-1.5 border-b border-border pb-1">
                    <AlertTriangle className="w-3.5 h-3.5" />
                    <span>Damaged Assets ({reportData.discrepancies.damaged.length})</span>
                  </h4>
                  {reportData.discrepancies.damaged.length === 0 ? (
                    <div className="text-xs text-text-muted italic p-2">No assets marked damaged.</div>
                  ) : (
                    <div className="border border-border rounded overflow-hidden">
                      <table className="w-full text-left text-xs border-collapse">
                        <tr className="bg-[#F7F8FA] border-b border-border font-bold">
            ...
            {isManager && reportData.summary.pendingCount === 0 && (
              <div className="px-6 py-4 bg-[#F7F8FA] border-t border-border flex justify-between items-center shrink-0">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-danger hover:bg-danger/90 text-white rounded-sm text-xs font-semibold shadow-sm transition-colors"
                >
